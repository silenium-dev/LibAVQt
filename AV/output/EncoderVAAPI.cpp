...
namespace AV {
    ...
    void EncoderVAAPI::onFrame(QImage frame, AVRational timebase, AVRational framerate) {
        ...
        if (d->m_cbType == 0 || d->m_cbType == IFrameSource::CB_QIMAGE) {
    ...
    void EncoderVAAPI::onFrame(AVFrame *frame, AVRational timebase, AVRational framerate) {
        Q_D(AV::EncoderVAAPI);
        QMutexLocker lock(&d->m_cbTypeMutex);
        if (d->m_cbType == 0 || d->m_cbType == IFrameSource::CB_AVFRAME) {
    ...
    void EncoderVAAPI::run() {
        ...
        while (d->isRunning.load()) {
            ...
            if (d->m_isFirstFrame.load()) {
                ...
                d->pVideoCodecCtx->framerate = queueFrame->framerate;
                d->pVideoCodecCtx->time_base = av_inv_q(d->pVideoCodecCtx->framerate);
                    ...
                    while (d->isRunning.load()) {
                        while (true) {
                            ...
                            if (ret != 0) {
                                ...
                                packet->duration = d->pVideoStream->time_base.num;
